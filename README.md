# tensorrt_demos - Microservices

This respository was build on top of another repository (), with some modifications, it was created a file 'trt_micro_service.py' wich basically is a copy of 'trt_yolo.py' with addition of some functions that call other microservices. 
Originally the 'trt_yolo.py' file just open a screen and perform image capture with yolov4, but with the modifications that i've made when a press the 'space' key the script take a picture of the realtime recording, execute the yolov4, save the images into a paste with some informations in a .txt file and finally calls the microservices.
About the microservices, it's made a post request in the API provided, that request post the images and some informations that was generated before.

## Running the yolov4 + microservicesw

To run this aplication is necessary to follow some steps:

1. Install version "1.4.1" (not the latest version) of python3 "onnx" module. Note that the "onnx" module would depend on "protobuf" as stated in the Prerequisite section. Reference: information provided by NVIDIA.
...
3. Download the pre-trained yolov3/yolov4 COCO models and convert the targeted model to ONNX and then to TensorRT engine.
...
* it's necessary to have an webcam set on port 0.
* Press 'space' to capture the image.
* Press 'esc' to close the aplication.

```
   python3 trt_micro_service.py --usb 0 -m yolov4-tiny-416
```
